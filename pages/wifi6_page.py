...
class Wifi6Page(BasePage):
    """Page Object for the WiFi 6GHz Details Page."""

    # URL
    URL_PATH = "#wifi/details/private:6"

    # Locators (Assuming identical structure to 5GHz)
    ...
    def navigate(self):
        """Navigates to the WiFi 6GHz details page."""
        # Using 192.168.3.5 as it's the expected IP after LAN config
        url = f"http://192.168.3.5/{self.URL_PATH}"
        self.logger.info(f"Navigating to WiFi 6GHz details page: {url}")
        self.driver.get(url)
        self.wait_for_page_load()

    def update_ssid_and_password(self, ssid, password):
        """Changes the SSID and Password with clearing."""
        self.logger.info(f"Changing SSID to {ssid} and Password to {password} for WiFi 6GHz")
        try:
            # SSID
            ...
            self.logger.warning("Recovery happened during WiFi 6GHz SSID/Password update.")
            return False

    def select_security_wpa3(self):
        """Selects WPA3 Personal security mode and clicks Save."""
        self.logger.info("Selecting WPA3 Personal security mode for WiFi 6GHz")
        try:
            # 1. Open menu
            ...
            # 5. Re-navigate
            self.logger.info("Re-navigating to WiFi 6GHz details page...")
            self.navigate()
            
            return True
        except RecoveryHandledException:
            self.logger.warning("Recovery happened during WiFi 6GHz security mode selection/save.")
            return False

    def select_first_device_and_apply(self):
        """Clicks device input, chooses the first device if any, clicks Add, then Apply."""
        self.logger.info("Attempting to select the first available device for WiFi 6GHz, add it, and apply")
        try:
            ...
            if options:
                self.logger.info(f"Found {len(options)} device options for WiFi 6GHz. Selecting the first one.")
                ...
                self.logger.info("No device entries found in the list for WiFi 6GHz.")
                return True
        except RecoveryHandledException:
            self.logger.warning("Recovery happened during WiFi 6GHz device selection/addition.")
            return False
        except Exception as e:
            self.logger.error(f"Failed to select/add device for WiFi 6GHz: {e}")
            return False
    ...
    def toggle_radio_and_apply(self, index=0, locator=None):
        ...
        self.logger.info(f"Toggling radio/checkbox (locator: {target_loc}, index: {index}) for WiFi 6GHz and applying")
        ...
        try:
            if locator:
                ...
                if len(toggles) > index:
                    ...
                    self.logger.warning(f"No radio/checkbox found at index {index} for WiFi 6GHz")
                    return False
            ...
            self.logger.warning(f"Recovery happened during WiFi 6GHz radio toggle at index {index}.")
            return False
        except Exception as e:
            self.logger.error(f"Failed to toggle radio for WiFi 6GHz: {e}")
            return False
